...
function generateBrandConfig(answers) {
    const appId = answers.projectName.toLowerCase().replace(/[^a-z0-9]+/g, '-');
...
export const brand = {
  ...
  email: {
    fromSupport: "${answers.projectName} Support <${answers.email}>",
    fromBilling: "${answers.projectName} Billing <billing@example.com>",
    fromNoReply: "${answers.projectName} <noreply@example.com>",
  ...
  inngest: {
    appId: process.env.INNGEST_APP_ID || "${appId}",
    appName: "${answers.projectName} Background Jobs",
  },
...
`;
    ...
    log('✓ Created src/config/brand.ts', 'green');
}
...
`;
    ...
    fs.writeFileSync(filePath, content, 'utf8');
    log('✓ Created src/config/theme.ts', 'green');
}
...
function updateLayoutThemeImport(answers) {
    ...
    if (!fs.existsSync(layoutPath)) {
        log('⚠ layout.tsx not found, skipping theme import update', 'yellow');
        return;
    }
    ...
    if (themeImportRegex.test(layoutContent)) {
        ...
        log('✓ Updated layout.tsx theme import', 'green');
    } else {
        log('⚠ Could not find theme import in layout.tsx', 'yellow');
    }
}
...
function copyBlueprintFiles(blueprintType, forceOverwrite = false) {
    ...
    if (!fs.existsSync(blueprintDir)) {
        log(`⚠ Blueprint directory not found: ${blueprintDir}`, 'yellow');
        return;
    }
    ...
    if (!hasContent) {
        log(`ℹ "${blueprintType}" blueprint is minimal - no files to copy`, 'cyan');
        return;
    }

    log(`\n📦 Copying "${blueprintType}" blueprint files...`, 'cyan');
    ...
    if (fs.existsSync(appDir)) {
        ...
        if (result.copiedFiles.length > 0) {
            log(`  ✓ Copied ${result.copiedFiles.length} app files`, 'green');
            totalCopied += result.copiedFiles.length;
        }
        if (result.skippedFiles.length > 0 && !forceOverwrite) {
            log(`  ⊘ Skipped ${result.skippedFiles.length} existing files`, 'yellow');
    ...
    if (fs.existsSync(dashboardDir)) {
        ...
        if (result.copiedFiles.length > 0) {
            log(`  ✓ Copied ${result.copiedFiles.length} dashboard files`, 'green');
            totalCopied += result.copiedFiles.length;
        }
        if (result.skippedFiles.length > 0 && !forceOverwrite) {
            log(`  ⊘ Skipped ${result.skippedFiles.length} existing files`, 'yellow');
    ...
    if (fs.existsSync(componentsDir)) {
        ...
        if (result.copiedFiles.length > 0) {
            log(`  ✓ Copied ${result.copiedFiles.length} component files to src/components/${blueprintSlug}/`, 'green');
            totalCopied += result.copiedFiles.length;
        }
        if (result.skippedFiles.length > 0 && !forceOverwrite) {
            log(`  ⊘ Skipped ${result.skippedFiles.length} existing files`, 'yellow');
    ...
    if (fs.existsSync(migrationFile)) {
        console.log('');
        log('📄 Database Migration Required:', 'yellow');
    ...
    if (totalSkipped > 0 && !forceOverwrite) {
        console.log('');
        log(`ℹ To overwrite existing files, run: node scripts/setup-blueprint.js --force`, 'cyan');
    }
...
function createBlueprintJson(answers) {
    ...
    log('✓ Created blueprint.json', 'green');
}
...
`;
    ...
    fs.writeFileSync(filePath, content, 'utf8');
    log('✓ Created .claude/PROJECT_CONTEXT.md', 'green');
}
...
function showWelcome() {
    console.log('');
    log('═══════════════════════════════════════════════════════', 'cyan');
    log('         🚀 PropelKit Setup Wizard 🚀', 'bright');
    log('═══════════════════════════════════════════════════════', 'cyan');
    console.log('');
...
async function setup(options = {}) {
    ...
    if (isAlreadySetup()) {
        ...
        if (!isPostinstall) {
            log('⚠ Setup already completed!', 'yellow');
    ...
    if (forceOverwrite) {
        log('⚠ Running in FORCE mode - will overwrite existing files\n', 'yellow');
    }
    ...
    try {
        ...
        console.log('');
        log('⚙️  Setting up your project...', 'cyan');
        console.log('');
        ...
        if (error.isTtyError) {
            log('✗ Prompt couldn\'t be rendered in this environment', 'red');

...
import { createClient } from "@/lib/supabase-browser"; // Import Supabase
import { UserNav } from "@/components/UserNav"; // Import the new component

// ✅ UPDATED: Added 'Live Demo' and 'Docs'
const navLinks = [
  { href: "/#features", label: "Features" },
  { href: "/admin-demo", label: "Live Demo" },
...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    // 3. Listen for Auth Changes (Login/Logout updates UI instantly)
        ...
        <Link href="/" className="flex items-center gap-2 group">
          <div className="relative flex items-center justify-center w-8 h-8">
            ...
            <img
              ...
              style={{ filter: "brightness(0) saturate(100%) invert(88%) sepia(21%) saturate(6654%) hue-rotate(357deg) brightness(103%) contrast(104%)" }}
            />
          </div>
          <span className="font-bold text-xl text-foreground hidden sm:block">PropelKit</span>
        </Link>
          ...
          {user ? (
            ...
            <>
              ...
              <Link href="/#pricing" onClick={() => setIsMobileMenuOpen(false)}>
                <Button className="w-full gradient-primary rounded-xl">Get PropelKit</Button>
              </Link>
            </>
          )}
        </div>
